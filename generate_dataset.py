        ...
        while retry_count < max_retries_per_episode and not episode_success:
            ...
            if args.get('pretrain_mode'):
                ...
                if not left_candidates or not right_candidates:
                    # Fallback if shift/spacing pushes everything too far (unlikely with current params)
                    print("WARNING: Could not find candidates in both regions! Fallback to random.")
                    indices = np.random.choice(10, 2, replace=False)
                else:
                    l_idx = np.random.choice(left_candidates)
                    r_idx = np.random.choice(right_candidates)
                    indices = np.array([l_idx, r_idx]) # Order doesn't matter for independent R assignment
                
                print(f"  [Gen] Spatial Selection. Left(x<-0.1): {l_idx} (x={xs[l_idx]:.3f}), Right(x>0): {r_idx} (x={xs[r_idx]:.3f})")
                ...
                if args.get('pretrain_mode') == 'independent':
                    ...
                    # Cooperative: Commands = C or CT
                    command_sequence_str = np.random.choice(["C", "CT"])
                    command_queue_template = list(command_sequence_str)
